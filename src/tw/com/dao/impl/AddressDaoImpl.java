...
public class AddressDaoImpl implements IAddressDao {
	...
	public boolean delAddress(String id) {
		System.out.println("Dao層: delAddress()...");
		Session session = getCurrentSession();
		Transaction ts = session.beginTransaction();
	...
	public boolean updateAddress(Address address) {
		System.out.println("Dao層: updateAddress()...");
		Session session = getCurrentSession();
		Transaction ts = session.beginTransaction();
		String hql = "update Address set accept=:accept,city=:city,area=:area"
				+ ",address=:address,phoneNum=:phoneNum,zip=:zip where id=:id";
		int rs = session.createQuery(hql).setParameter("id", address.getId())
				.setParameter("accept", address.getAccept()).setParameter("city", address.getCity())
				.setParameter("area", address.getArea()).setParameter("address", address.getAddress())
				.setParameter("phoneNum", address.getPhoneNum()).setParameter("zip", address.getZip()).executeUpdate();
		ts.commit();
		return rs != 0;
	}
